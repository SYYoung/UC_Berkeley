...
def print_kth_top_movie_year(k):
    ...
    rank_from_that_year = movies_by_year.sort("Total Gross", descending=True).column('Year').item(k-1)
    print("Year number", k, "for total gross movie sales was:", rank_from_that_year)
...
having_previous_year = having_previous_year.relabel('Total Pay ($)', '2014 Total Pay ($)')

# Get the percent changes as numbers instead of strings
percent_changes = having_previous_year.apply(percent_string_to_num, '% Change')

# Calculate the previous years pay
previous_pay = having_previous_year.column('2014 Total Pay ($)') /(1 + percent_changes/100)
...
average_pay_2014 = np.average(having_previous_year.column('2014 Total Pay ($)'))
average_pay_2014
